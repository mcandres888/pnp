...
<form method="POST" action="<?=site_url()?>/main/sent_filtered">
				...
				<table class="table table-bordered table-striped table-hover table-heading table-datatable" id="datatable-2">
					<thead>
						<tr>
							...
							<th><label><input type="text" name="status" value="Status" class="search_init" /></label></th>
							<th><label><input type="text" name="sender" value="Status" class="search_init" /></label></th>
							<th>Action</th>
						</tr>
					</thead>
					<tbody>
...
<?php
  ...
  $site_ = site_url(); 
	foreach ($messages as $message) {
		...
		echo "<td>" . $message->TextDecoded . "</td>";
		echo "<td>" . $message->SendingDateTime . "</td>";
		echo "<td>" . $message->Status . "</td>";
		echo "<td>" . $message->SenderID . "</td>";
    echo "<td><a href='" . $site_ . "/main/delete_sentitem/" . $message->ID .  "'>Delete<a/></td>";
