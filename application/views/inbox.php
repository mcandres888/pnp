...
<form method="POST" action="<?=site_url()?>/main/inbox_filtered">
...
<?php
	...
	foreach ($messages as $message) {
		echo "<tr>";
		echo "<td><a href='" . $site_ . "/main/view_temp/" . $message->ID . "'> <i class='fa fa-envelope'></i>  " . $message->group_name . "</a></td>";
    if ($message->Processed == 'true') {
			echo "<td>" . $message->contact_name . "</td>";
		} else {
			echo "<td><b>" . $message->contact_name . "</b></td>";
		}
		echo "<td>" . $message->SenderNumber . "</td>";
		echo "<td>" . $message->TextDecoded . "</td>";
		echo "<td>" . $message->ReceivingDateTime . "</td>";
		echo "<td><a href='" . $site_ . "/main/delete_inbox/" . $message->ID .  "'>Delete<a/></td>";
