...
class Inbox extends CI_Model {
  ...
  function getAll () {
    $query = $this->caller->db->query("SELECT * FROM $this->table_name ORDER BY ReceivingDateTime");
    ...
    foreach ($query->result() as $row) {
			...
			$temp = $contact->findContactNumber($row->SenderNumber)	;
			if (is_null($temp)) {
				$row->group_name = "none";
      ...
      if ( strlen($row->TextDecoded) > 200) {
          $row->TextDecoded = substr($row->TextDecoded, 0, 200) . "...[more]";
      }
      $data[] = $row;
    }
    return $data;
  ...
  function getUnreadCount () {
   $query = $this->caller->db->query("SELECT * FROM $this->table_name WHERE Processed='false'");
    $total = $this->caller->db->affected_rows();
 		return $total; 
	}
  ...
  function getMessage ( $message_id) {
    $query_temp = $this->caller->db->query("UPDATE $this->table_name SET Processed='true' WHERE ID=$message_id");
    $query = $this->caller->db->query("SELECT * FROM $this->table_name WHERE ID=$message_id LIMIT 1");
    $total = $this->caller->db->affected_rows();
    $data = array();
		$this->caller->load->model('contact');
		$contact = new contact();
  ...
  function getAllFilteredAsCSV (  ) {
    ...
    if ($search_number != "Search number") {
			$filter_arr[] = " SenderNumber LIKE '%". $search_number ."%' ";
		}
    ...
    if ($search_date != "Search date") {
			$filter_arr[] = " ReceivingDateTime LIKE '%". $search_date ."%' ";
		}
    ...
    foreach ($query->result() as $row) {
      ...
      $data[] = $row;
			$csv_str .= $x . "," . $row->group_name . "," . $row->contact_name . "," . $row->SenderNumber . ",\"" . $row->TextDecoded . "\"," . $row->ReceivingDateTime ."\n";
			$x = $x + 1;
    }
    return $csv_str;
  }
