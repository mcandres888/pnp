...
class SentItems extends CI_Model {
	
    
    var $caller ;
    var $table_name = 'sentitems';
  ...
  function getAll () {
    ...
    foreach ($query->result() as $row) {
      ...
      if ( $row->ID == $last_id) {
         ...
         $data[$lastindex]->TextDecoded .= $row->TextDecoded;
         continue;
      }
      ...
      if ($row->Status == "SendingError" ) {
         $row->Status = "No Load?";
      }
 
      if ( strlen($row->TextDecoded) > 80) {
          $row->TextDecoded = substr($row->TextDecoded, 0, 80) . "...[more]";
      }
      $last_id = $row->ID;
      $data[] = $row;
    }
